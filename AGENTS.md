...
JACS is a portable cryptographic primitive. The active Cargo workspace contains exactly jacs-core, jacs-wasm, jacs-mobile, jacs-mcp and jacs-cli. Do not reintroduce archived native integrations into their dependency graph. Preserve licenses and compatibility source in archive/native; its AGENTS.md applies to work there.

Document MCP as the primary integration. Node, Python and Go bindings remain
buildable and publishable through the root Makefile and CI, using their separate
native workspace. Coordinate all 17 Rust crates, Python, Go and both npm packages
at the same source version. The native MCP/CLI packages use jacs-mcp-compat and
jacs-cli-compat to preserve the portable package identities; Rust dependency
aliases retain existing source imports. The compatibility CLI binary is jacs-compat.
...
Validate affected behavior with focused tests, then required workspace/browser/mobile gates. Run cargo fmt on files touched. Use make check for dependency, release, licensing, notices and security-policy checks. Release all crates in scripts/release_catalog.py dependency order; keep exact candidate verification, registry checksums, audit gates, SBOMs and provenance. Source changes must not be represented as already published releases.
