...
## Evidence
File: `jacs-cli/tests/cli_command_snapshot.rs:1` - CLI test validates command fixture parity against the Clap tree.
File: `jacs-cli/tests/cli_command_snapshot.rs:72` - the main assertion compares fixture command paths to Clap command paths, not behavior.
File: `jacs-mcp/tests/tool_surface.rs:111` - MCP full-tools test validates the registered tool names.
File: `jacs-mcp/tests/contract_snapshot.rs:12` - MCP contract test compares generated schema snapshot to checked-in artifact.
PRD: the Agreement v2 schema PRD (internal) - calls for developer workflows and functions across CLI/MCP surfaces, not just command discovery.
## Suggested Fix
