...
## Evidence
File: `binding-core/tests/fixtures/method_parity.json:13` - agreement v2 canonical method list exists for binding surfaces.
File: `jacs-wasm/tests/native_sanity.rs:130` - WASM agreement v2 test covers only create/sign/verify happy path.
File: `jacs-wasm/jacs_wasm.d.ts:44` - TypeScript declaration lists agreement v2 methods, but there is no fixture-based drift test analogous to Python/Node/Go method parity.
PRD: the Agreement v2 schema PRD (internal) - includes a human signing workflow using the JACS WASM signer, making WASM parity a product-critical surface.
## Suggested Fix
