...
async function jacsSignTransform(message: JSONRPCMessage): Promise<string> {
    ...
    try {
        ...
        if (enableDiagnosticLogging) console.log(`jacsSignTransform: Output FROM jacs.signRequest (type ${typeof jacs_artifact}, length ${typeof jacs_artifact === 'string' ? jacs_artifact.length : 'N/A'}): ${String(jacs_artifact).substring(0,150)}...`);
        ...
        if (enableDiagnosticLogging) console.log(`jacsSignTransform: SUCCESSFULLY SIGNED. Returning JACS string of length ${result.length}`);
        return result;
...
async function jacsVerifyTransform(jacsArtifactString: string): Promise<JSONRPCMessage> {
    if (!jacsLoaded) {
    ...
    try {
        if (enableDiagnosticLogging) console.log(`jacsVerifyTransform: Input TO jacs.verifyResponse (type ${typeof jacsArtifactString}, length ${jacsArtifactString.length}): ${jacsArtifactString.substring(0,150)}...`);
        
        // Verify the JACS artifact string and get back the original JSON-RPC message
        const verificationResult = await jacs.verifyResponse(jacsArtifactString); 
        ...
        if (verificationResult && typeof verificationResult === 'object') {
            ...
            if ('payload' in verificationResult) {
                ...
                if (enableDiagnosticLogging) console.log(`jacsVerifyTransform: Extracted payload from verificationResult.payload`);
            } else {
                // If verifyResponse returns the payload directly
                originalMessage = verificationResult as JSONRPCMessage;
                if (enableDiagnosticLogging) console.log(`jacsVerifyTransform: Using verificationResult directly as originalMessage`);
            }
        } else {
...
export class TransportMiddleware implements Transport {
  ...
  constructor(
    ...
    private incomingJacsTransformer?: (payload: string) => Promise<JSONRPCMessage>,
      ...
      try {
        if (typeof messageOrStringFromTransport === 'string') {
            ...
            if (this.incomingJacsTransformer && this.jacsOperational) {
                if (enableDiagnosticLogging) console.log(`${startLogPrefix}: JACS operational, applying incomingJacsTransformer (string->obj).`);
                ...
                if (enableDiagnosticLogging) console.log(`${startLogPrefix}: JACS not operational, parsing string as JSON.`);
        ...
        if (enableDiagnosticLogging) console.log(`${startLogPrefix}: Final message object prepared: ${JSON.stringify(messageObject).substring(0,100)}...`);
  ...
  async send(message: JSONRPCMessage): Promise<void> {
    ...
    try {
      let payloadToSend: string | JSONRPCMessage = message;
      ...
      if (this.outgoingJacsTransformer && this.jacsOperational) {
        if (isJSONRPCResponse(message) && 'error' in message) {
            ...
            if (enableDiagnosticLogging) console.log(`${startLogPrefix}: JACS operational, applying outgoingJacsTransformer (obj->string).`);
            payloadToSend = await this.outgoingJacsTransformer(message); 
            if (enableDiagnosticLogging) console.log(`${startLogPrefix}: outgoingJacsTransformer completed. Result type: ${typeof payloadToSend}, length: ${typeof payloadToSend === 'string' ? payloadToSend.length : 'N/A'}`);
        }
      } else {
          if (enableDiagnosticLogging) console.log(`${startLogPrefix}: JACS not operational, sending original message object.`);
      }
      ...
      if (enableDiagnosticLogging) console.log(`${startLogPrefix}: Calling underlying transport.send() with payload type: ${typeof payloadToSend}`);
      await this.transport.send(payloadToSend as any); 
  ...
  async handlePostMessage(req: IncomingMessage & { auth?: any }, res: ServerResponse, rawBodyString?: string): Promise<void> {
    ...
    try {
        ...
        if (this.jacsOperational && this.incomingJacsTransformer) {
            if (enableDiagnosticLogging) console.log(`${logPrefix}: JACS operational. Calling incomingJacsTransformer (string->obj).`);
            ...
            if (enableDiagnosticLogging) console.log(`${logPrefix}: JACS not operational. Parsing POST body as JSON.`);
