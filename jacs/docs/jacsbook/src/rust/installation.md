...
`0.11.3`; the documentation source is `0.13.0`. Check the resolved version
...
`release/shipped-artifacts.json`; source checkout `0.13.0` is not installable
...
| Browser WASM | source `0.13.0` only | No published `@jacs/wasm` package | Source CI runs Firefox `wasm-pack` tests and a Chromium/Playwright package smoke on Ubuntu. |
