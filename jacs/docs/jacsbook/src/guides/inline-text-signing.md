...
| Field | What it is |
|-------|-----------|
| `signatureBlockVersion` | Always `1` for v0.11.0. Versioning the block lets the format evolve without breaking older verifiers. |
...
There is intentionally **no** `--force-overwrite-markers` escape hatch in v0.11.0. Its semantics under the first-marker-splits-content parser are ambiguous — easier to fix the input file than to ship a footgun.
