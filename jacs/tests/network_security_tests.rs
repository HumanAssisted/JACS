...
#[test]
#[serial]
fn verify_registry_registration_rejects_non_https_api_url() {
    // SAFETY: serial test; env var mutation is isolated for this test.
    unsafe {
        std::env::set_var("JACS_REGISTRY_URL", "ftp://example.com");
    }

    let result = verify_registry_registration_sync(TEST_AGENT_ID, "deadbeef");
...
#[test]
#[serial]
fn verify_registry_registration_allows_localhost_http_for_testing() {
    // SAFETY: serial test; env var mutation is isolated for this test.
    unsafe {
        std::env::set_var("JACS_REGISTRY_URL", "http://localhost:1");
    }
