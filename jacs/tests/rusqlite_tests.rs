...
/// Create a file-based rusqlite storage in a temp directory.
fn create_file_rusqlite(tmpdir: &TempDir) -> RusqliteStorage {
    let db_path = tmpdir.path().join("test.db");
    let db_path_str = db_path.to_str().expect("valid path");
    let storage = RusqliteStorage::new(db_path_str).expect("Failed to create file-based rusqlite");
...
async fn test_rusqlite_file_persistence() {
    ...
    {
        let storage = RusqliteStorage::new(&db_path_str).expect("Failed to create rusqlite");
    ...
    {
        let storage = RusqliteStorage::new(&db_path_str).expect("Failed to reopen rusqlite");
