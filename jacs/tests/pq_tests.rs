...
fn test_pq_create_and_verify_signature() {
    ...
    let _result = agent
        .create_agent_and_load(&json_data, create_keys, None)
        .expect("create_agent_and_load failed");

    // Keys are now loaded directly from create_agent_and_load
    println!(
        "Keys status: {}",
    ...
    // Decrypt and test
    let key_vec = decrypt_private_key_secure(borrowed_key).expect("Failed to decrypt key");
    println!(
        "Decryption successful!\nKey details:\n- Private key length: {} bytes\n- Public key length: {} bytes",
        key_vec.as_slice().len(),
        public.len()
    );
