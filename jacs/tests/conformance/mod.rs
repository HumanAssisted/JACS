//! Conformance test harness for JACS storage backends.
//!
//! Provides two macros that generate standard test suites:
//! - `storage_conformance_tests!` — tests for `StorageDocumentTraits` (11 tests)
//! - `database_conformance_tests!` — tests for `DatabaseDocumentTraits` (9 tests)
//!
//! Each macro takes a factory closure that returns the backend instance.
//!
//! Note: `list_documents` and `get_documents_by_agent` have different semantics
//! between `MultiStorage` (file path prefix / document ID match) and database
//! backends (jacs_type filter / agent_id column). These are tested in
//! `database_conformance_tests!` only.
