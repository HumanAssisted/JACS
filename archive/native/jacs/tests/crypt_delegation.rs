...
const FIXTURE_ED25519_PUBLIC: &[u8] = include_bytes!("fixtures/wasm_compat/ed25519.public.bin");
...
fn delegation_preserves_ring_output() {
    ...
    let wrapper_sig =
        ringwrapper::sign_string(private.to_vec(), &canonical).expect("ringwrapper sign");
    let wrapper_sig_bytes = B64.decode(&wrapper_sig).expect("decode wrapper sig");

    let direct = Ed25519DalekSigner::from_pkcs8(&private).expect("direct import");
    ...
    ringwrapper::verify_string(public, &canonical, &wrapper_sig).expect("runtime verify");
    // Preserve the historical ring-era verification oracle without its key.
    let fixture_sig = B64
        .decode(parsed["signature_b64"].as_str().expect("sig_b64"))
        .expect("base64");
    Ed25519DalekSigner::verify(FIXTURE_ED25519_PUBLIC, canonical.as_bytes(), &fixture_sig)
        .expect("fixed ring-era public oracle verifies");
}
