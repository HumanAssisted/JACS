...
//!   AES-256-GCM) wrapping the historical disposable Ed25519 key. Produced by the current
...
//!   wrapping the historical disposable Ed25519 key. The current writer no longer emits this
...
//! These are public, disposable interoperability vectors inherited from
//! commit 992953e; never use them as identities. Raw private-key files are not
//! retained. Signing tests generate fresh in-memory keys; decrypt tests derive
//! and compare the preserved public-key oracle.
//!
//! ## Regenerating
//!
//! ```bash
...
/// Keep the original public verification oracle independent of runtime signing.
#[test]
fn ed25519_fixture_verify_and_runtime_sign_roundtrip() {
    let public_key = read_fixture("ed25519.public.bin");
    let signed = read_fixture_json("ed25519.signed.json");
    let canonical = signed["canonical"].as_str().expect("canonical string");
    let expected = signed["signature_b64"].as_str().expect("signature_b64");
    ringwrapper::verify_string(public_key, canonical, expected).expect("fixed oracle verifies");

    let (private, public) = ringwrapper::generate_keys().expect("fresh key");
    let private = zeroize::Zeroizing::new(private);
    let first = ringwrapper::sign_string(private.to_vec(), &canonical.to_string()).expect("sign");
    let second = ringwrapper::sign_string(private.to_vec(), &canonical.to_string()).expect("sign");
    assert_eq!(first, second, "Ed25519 remains deterministic");
    ringwrapper::verify_string(public, canonical, &first).expect("runtime signature verifies");
}

fn assert_fixture_public_key(private: &[u8]) {
    use jacs_core::sign::{DetachedSigner, Ed25519DalekSigner};
    let signer = Ed25519DalekSigner::from_pkcs8(private).expect("decoded Ed25519 key");
    assert_eq!(signer.public_key(), read_fixture("ed25519.public.bin"));
}
...
mod regen {
    ...
    pub fn regenerate_all() {
        ...
        let pkcs8 = write_ed25519_fixtures();
        write_pq2025_fixtures();
        write_canonical_fixtures();
        // Only public vectors and encrypted test envelopes are written.
        // The fresh private key remains in memory and is zeroized on return.
        write_argon2id_envelope(&pkcs8);
        write_pbkdf2_envelope(&pkcs8);
    ...
    fn write_ed25519_fixtures() -> zeroize::Zeroizing<Vec<u8>> {
        ...
        let pkcs8 = zeroize::Zeroizing::new(pkcs8);
        let payload = json!({"fixture": "wasm_compat::ed25519", "ts": "2026-05-16"});
        let canonical = canonicalize_json(&payload);
        let sig_b64 = ringwrapper::sign_string(pkcs8.to_vec(), &canonical).expect("sign");
    ...
    fn write_argon2id_envelope(pkcs8: &[u8]) {
    ...
    fn write_pbkdf2_envelope(pkcs8: &[u8]) {
