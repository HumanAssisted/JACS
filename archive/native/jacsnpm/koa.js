...
function jacsKoaMiddleware(options = {}) {
    ...
    return async function jacsKoaMiddlewareHandler(ctx, next) {
        ...
        if (shouldVerify && BODY_METHODS.has(ctx.method)) {
            ...
            const verificationInput = ctx.request.body !== undefined
                ? ctx.request.body
                : typeof ctx.body !== 'undefined' && ctx.method !== 'GET'
                    ? ctx.body
                    : undefined;
            const rawBody = typeof verificationInput === 'string'
                ? verificationInput
        ...
        if (shouldSign && typeof ctx.body !== 'undefined' && !Buffer.isBuffer(ctx.body)) {
            ...
            try {
                ...
                ctx.body = (0, output_policy_js_1.requireSignedRaw)(signed, 'JACS Koa response signing');
                ctx.type = 'application/json';
            }
            catch (error) {
