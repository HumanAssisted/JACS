...
# Configuration
JACS_CONFIG_PATH = os.environ["JACS_CONFIG_PATH"]

# Initialize JACS
jacs.load(JACS_CONFIG_PATH)

# Create MCP server with JACS authentication
ALLOWED_CLIENT_AGENT_ID = os.environ["JACS_MCP_ALLOWED_CLIENT_AGENT_ID"]
mcp = JACSMCPServer(
    FastMCP("JACS A2A Agent"),
    JACS_CONFIG_PATH,
...
def analyze_document(
    ...
    result = {
        ...
        "results": {
            ...
            if "entity-extraction" in operations
            else None,
            "classification": {"category": "business-document", "confidence": 0.89}
...
def get_well_known_document(path: str) -> Dict[str, Any]:
    ...
    if not well_known_docs:
        ...
        well_known_docs = a2a_integration.generate_well_known_documents(
            agent_card, jws_signature, public_key_b64, agent_data
        )
...
def create_workflow_with_provenance(
    ...
    for i, step in enumerate(steps):
        ...
        wrapped_step = a2a_integration.wrap_artifact_with_provenance(
            step, "workflow-step", parent_sigs
        )
