...
def _metadata_components(
    ...
    # The archived workspace uses the active portable core through a path
    # dependency. It is first-party even though it is not an archive workspace
    # member. Bind this exception to the exact source manifest, never its name
    # alone: a registry package or another local checkout must remain inventoried.
    core_manifest = (ROOT.parent.parent / "jacs-core/Cargo.toml").resolve()
    for package_id, raw_package in packages_by_id.items():
        manifest_path = raw_package.get("manifest_path")
        if (
            raw_package.get("name") == "jacs-core"
            and raw_package.get("source") is None
            and isinstance(manifest_path, str)
            and Path(manifest_path).is_absolute()
            and Path(manifest_path).resolve() == core_manifest
        ):
            first_party_ids.add(package_id)
...
def render_inventory(components: Sequence[Component]) -> str:
    ...
    lines = [
        ...
        "Source graphs: locked all-feature archived workspace plus excluded archived crates.",
        "First-party packages are excluded by exact workspace IDs or the local core manifest.",
