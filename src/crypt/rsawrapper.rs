...
fn load_private_key_from_file<P: AsRef<Path>>(file_path: P) -> Result<RsaPrivateKey, Box<dyn std::error::Error>> {
    let pem = std::fs::read_to_string(file_path)?;
    let private_key = RsaPrivateKey::from_pkcs8_pem(&pem)?;
    Ok(private_key)
}

fn load_public_key_from_file<P: AsRef<Path>>(file_path: P) -> Result<RsaPublicKey, Box<dyn std::error::Error>> {
    let pem = std::fs::read_to_string(file_path)?;
...
pub fn generate_keys(outputpath: &'static str) ->  Result<(String, String), Box<dyn std::error::Error>> {
    ...
    let private_key_path = super::save_file(outputpath, &private_key_filename, private_key_pem.as_bytes())?;
    let public_key_path = super::save_file(outputpath, &public_key_filename, public_key_pem.as_bytes())?;
