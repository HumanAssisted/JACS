...
Agreement v2 hardening from a multi-agent security review of the agreement vertical (each fix ships with a regression test):

- **Portable/WASM agreement verification is now cryptographic.** `jacs_core::agreements::v2::verify` — the only verify path in browser/WASM builds — previously returned `valid:true` from agreement-hash and status checks alone, never verifying any `agreementSignatures` entry and counting any `agentID` toward quorum, so a fully forged `final` agreement self-validated in the browser. It now verifies each signature against caller-supplied signer keys (mirroring the v1 detached path), counts only listed party members of the matching role toward quorum, and reports an explicit `verificationDepth`. The portable document verifier also gained the native unsigned-top-level-field rejection (SV-5).
- **Consent signatures bind the agreement identity.** A party signature covered only the consent-fields hash, so it could be transplanted onto a different agreement with identical terms/parties. `jacsId` is now part of the signed scope on both the native and portable paths, binding each signature to its agreement instance.
- **Quorum can no longer be unilaterally lowered.** A single signer-controller could rewrite `signaturePolicy` to `partyQuorum:1`, clear signatures, sign once, and reach `final`. Loosening the signature policy (lower quorum, fewer required witnesses/notaries, weaker minimum strength, broadened algorithms) is now rejected once an agreement is proposed, signed, or versioned.
- **Read-only verification is side-effect free; merges no longer launder forged signatures.** `verify`/`detect-conflict` previously persisted the caller-supplied document to storage before producing a verdict, and `merge`/`resolve` re-signed successors without checking carried signatures. Read-only operations now validate in memory without storing, `create` rejects caller-supplied signatures, and branch merge/resolve verify every carried signature before emitting a successor.
