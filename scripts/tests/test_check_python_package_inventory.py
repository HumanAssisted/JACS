...
class PythonPackageInventoryTests(unittest.TestCase):
    ...
    canonical_notices = (ROOT / "archive/native/THIRD-PARTY-NOTICES").read_bytes()

    def test_jacspy_license_copy_matches_repository_canonical_license(self) -> None:
        self.assertEqual(
            (ROOT / "archive/native/jacspy" / "LICENSE-APACHE").read_bytes(),
            self.canonical_license,
        )
        self.assertEqual(
            (ROOT / "archive/native/jacspy" / "THIRD-PARTY-NOTICES").read_bytes(),
            self.canonical_notices,
        )

    def test_portable_notice_override_is_limited_to_one_exact_path(self) -> None:
        with TemporaryDirectory() as directory:
            sdist = Path(directory) / "jacs-1.0.0.tar.gz"
            portable = b"portable dependency inventory\n"
            portable_path = "jacs-1.0.0/jacs-core/THIRD-PARTY-NOTICES"
            with tarfile.open(sdist, "w:gz") as archive:
                for name, data in (
                    ("jacs-1.0.0/LICENSE-APACHE", self.canonical_license),
                    ("jacs-1.0.0/THIRD-PARTY-NOTICES", self.canonical_notices),
                    (portable_path, portable),
                ):
                    info = tarfile.TarInfo(name)
                    info.size = len(data)
                    archive.addfile(info, io.BytesIO(data))
            self.assertEqual(inventory.inventory_errors(
                sdist, self.canonical_license, self.canonical_notices,
                {portable_path: portable}), [])
            for override in ({portable_path: b"changed"},
                             {"jacs-1.0.0/another/THIRD-PARTY-NOTICES": portable}):
                self.assertTrue(inventory.inventory_errors(
                    sdist, self.canonical_license, self.canonical_notices, override))

    def test_notice_override_rejects_unsafe_or_non_notice_paths(self) -> None:
        for name in ("../THIRD-PARTY-NOTICES", "/THIRD-PARTY-NOTICES", "jacs/LICENSE"):
            with self.subTest(name=name):
                errors = inventory.inventory_errors(
                    Path("absent.tar.gz"), self.canonical_license, self.canonical_notices,
                    {name: b"notices"})
                self.assertIn("invalid canonical notice override path", errors[0])
    ...
    def test_pypi_release_checks_wheels_and_source_distribution(self) -> None:
        ...
        self.assertGreaterEqual(workflow.count("--notices archive/native/THIRD-PARTY-NOTICES"), 2)
        self.assertIn('--notice-override "jacs-${VERSION}/jacs-core/THIRD-PARTY-NOTICES=jacs-core/THIRD-PARTY-NOTICES"', workflow)
        self.assertIn("jacspy/dist/*.whl", workflow)
        self.assertIn("jacspy/dist/*.tar.gz", workflow)

        pyproject = (ROOT / "archive/native/jacspy/pyproject.toml").read_text(encoding="utf-8")
