...
def _expected_legal_bytes(
    ...
    name: str | None = None, notice_overrides: dict[str, bytes] | None = None,
) -> bytes | None:
    if kind == "license":
        return canonical_license
    if kind == "notice":
        if notice_overrides is not None and name in notice_overrides:
            return notice_overrides[name]
...
def _zip_inventory(
    ...
    notice_overrides: dict[str, bytes] | None = None,
) -> _ArchiveInventory:
            ...
            with archive.open(info, "r") as entry_stream:
                matches, total_read = _stream_entry(
                    ...
                    expected_legal_bytes=_expected_legal_bytes(
                        ...
                        normalized_name, notice_overrides,
                    ),
                )
...
def inventory_errors(
    ...
    try:
        ...
        for name, expected in (notice_overrides or {}).items():
            normalized, issues = _normalize_archive_path(name, is_directory=False)
            if issues or _legal_kind(normalized or "") != "notice":
                raise _ArchiveRejected(f"invalid canonical notice override path: {name!r}")
            if normalized in normalized_overrides:
                raise _ArchiveRejected(f"duplicate canonical notice override path: {name!r}")
            normalized_overrides[normalized] = expected
    ...
    errors.extend(f"canonical notice override path is missing: {name}"
                  for name in sorted(set(normalized_overrides) - set(archive.normalized_names)))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--notice-override", action="append", default=[], metavar="ARCHIVE_PATH=FILE",
        help="canonical notices for one exact archive path (for bundled portable crates)",
    )
    ...
    notice_overrides = {}
    for item in args.notice_override:
        name, separator, source = item.partition("=")
        if not separator or not name or not source or name in notice_overrides:
            parser.error("--notice-override requires a unique ARCHIVE_PATH=FILE")
        notice_overrides[name] = Path(source).read_bytes()
    failed = False
