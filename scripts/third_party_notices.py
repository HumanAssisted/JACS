...
# The archive owns an immutable historical notice snapshot. Only the active
# five-crate graph and its package copies are refreshed by this generator.
REQUIRED_PACKAGE_COPIES = tuple(
    Path(package) / "THIRD-PARTY-NOTICES"
    for package in ("jacs-core", "jacs-wasm", "jacs-mobile", "jacs-mcp", "jacs-cli")
)
...
def render_inventory(components: Sequence[Component]) -> str:
    ...
    lines = [
        ...
        "Source graph: locked all-feature active portable workspace; archived native integrations excluded.",
